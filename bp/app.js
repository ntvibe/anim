(() => {
  ...
  async function decodeAnimation(){if(window.__BP_DATA_GZIP){const bytes=Uint8Array.from(atob(window.__BP_DATA_GZIP),c=>c.charCodeAt(0));if(typeof DecompressionStream!=="undefined"){const stream=new Blob([bytes]).stream().pipeThrough(new DecompressionStream("gzip"));return JSON.parse(await new Response(stream).text());}throw new Error("This browser does not support DecompressionStream for the embedded Bitpanda animation data.");}const parts=window.__BP_DATA_PARTS||[];if(!parts.length||parts.some(x=>!x))throw new Error("Bitpanda Lottie data is incomplete.");const bytes=Uint8Array.from(atob(parts.join("")),c=>c.charCodeAt(0));return JSON.parse(new TextDecoder().decode(bytes));}
