...
function commitHistory() {
  ...
  if (history.length > 80) historyIndex = history.length - 1;
  syncHistoryButtons();
}
function restoreHistory(index) {
  ...
  syncAll({ reloadFont: true, reloadLogo: true });
  syncHistoryButtons();
}
...
function lifecycleState(localT, s, travel, startScale, wordIndex = 0, wordCount = 1) {
  ...
  const total = s.lifecycleDuration;
  const localMs = clamp(localT, 0, 1) * total;
  const enterVec = directionVector(s.enterDirection), exitVec = directionVector(s.exitDirection);
  if (s.flowMode === 'out-only') {
    const p = cubicBezierEase(clamp(localMs / Math.max(1, s.exitDuration), 0, 1), ...s.exitBezier);
    return { alpha: 1 - p, x: exitVec[0] * travel * p, y: exitVec[1] * travel * p, scale: 1 };
  }
  const stagger = wordCount > 1 ? wordIndex * s.wordStagger : 0;
  const enterStart = stagger;
  const enterEnd = Math.min(total, enterStart + s.enterDuration);
  if (localMs < enterEnd) {
    const p = cubicBezierEase(clamp((localMs - enterStart) / Math.max(1, enterEnd - enterStart), 0, 1), ...s.enterBezier);
    return { alpha: p, x: enterVec[0] * travel * (1 - p), y: enterVec[1] * travel * (1 - p), scale: lerp(startScale, 1, p) };
  }
...
function resolvedTextState(context, s, width) {
  ...
  const logo = logoDimensions(s), safeWidth = Math.max(40, width - Math.min(s.marginX, width / 2 - 1) * 2);
  const maxTextWidth = (s.position === 'left' || s.position === 'right') ? Math.max(40, safeWidth - logo.w - s.gap) : safeWidth;
  let size = s.fontSize, candidate = { ...s, fontSize: size };
  while (size > 10 && measureSpacedText(context, candidate.headline, candidate) > maxTextWidth) { size -= 1; candidate = { ...s, fontSize: size }; }
  return candidate;
}
function lockupLayout(context, s, width, height) {
  const logo = logoDimensions(s), textW = measureSpacedText(context, s.headline, s), textH = s.fontSize * 1.05;
  ...
  if (s.position === 'below' || s.position === 'above') {
    const blockW = Math.max(logo.w, textW), blockH = logo.h + s.gap + textH, left = cx - blockW / 2;
    ...
    const blockW = logo.w + s.gap + textW, blockH = Math.max(logo.h, textH), left = cx - blockW / 2, top = cy - blockH / 2;
    const alignY = (itemH) => s.verticalAlignment === 'start' ? top + itemH / 2 : s.verticalAlignment === 'end' ? top + blockH - itemH / 2 : cy;
    if (s.position === 'right') { logoX = left + logo.w / 2; textX = left + logo.w + s.gap + textW / 2; }
    else { textX = left + textW / 2; logoX = left + textW + s.gap + logo.w / 2; }
    logoY = alignY(logo.h); textY = alignY(textH);
  }
  return {
    logoX: logoX + s.logoOffsetX, logoY: logoY + s.logoOffsetY, textX: textX + s.textOffsetX, textY: textY + s.textOffsetY,
    logoW: logo.w, logoH: logo.h, textW, textH,
  };
}
function drawGuides(context, s, width, height) {
  ...
  if (s.showGrid) {
    ...
    for (let i = 1; i < 4; i++) { const x = width * i / 4, y = height * i / 4; context.beginPath(); context.moveTo(x, 0); context.lineTo(x, height); context.stroke(); context.beginPath(); context.moveTo(0, y); context.lineTo(width, y); context.stroke(); }
  }
  if (s.showMargins) {
    const mx = Math.min(s.marginX, width / 2 - 1), my = Math.min(s.marginY, height / 2 - 1);
    context.strokeStyle = 'rgba(255,255,255,.42)'; context.setLineDash([6, 6]); context.strokeRect(mx, my, Math.max(1, width - mx * 2), Math.max(1, height - my * 2)); context.setLineDash([]);
  }
  context.restore();
}
function renderFrame(t, targetCtx = ctx, width = canvas.width, height = canvas.height, options = {}) {
  const s = renderState(), scaleX = width / s.canvasWidth, scaleY = height / s.canvasHeight;
  targetCtx.save(); targetCtx.clearRect(0, 0, width, height); targetCtx.scale(scaleX, scaleY);
  ...
  const textState = resolvedTextState(targetCtx, s, baseW), layout = lockupLayout(targetCtx, textState, baseW, baseH);
  const logoMotion = lifecycleState(rangeProgress(t, s.logoStart, s.logoEnd), s, s.logoTravel, s.logoScale);
  ...
  words.words.forEach((word, i) => {
    const motion = lifecycleState(rangeProgress(t, s.textStart, s.textEnd), s, s.textTravel, s.textScale, i, words.words.length);
...
function aspectFromSvg(svgText) {
  const match = svgText.match(/viewBox=["']\s*[-\d.]+\s+[-\d.]+\s+([\d.]+)\s+([\d.]+)["']/i); if (!match) return null;
  const w = Number(match[1]), h = Number(match[2]); return w > 0 && h > 0 ? h / w : null;
}
...
async function loadBuiltInLogo(type = model.logoType) {
  ...
  try {
    ...
    const sourceSvg = await response.text(), parsedAspect = aspectFromSvg(sourceSvg), svg = colorizeSvg(sourceSvg, model.logoColor), blob = new Blob([svg], { type: 'image/svg+xml' }), url = URL.createObjectURL(blob), image = new Image();
    await new Promise((resolve, reject) => { image.onload = resolve; image.onerror = reject; image.src = url; });
    if (token !== logoLoadToken) { URL.revokeObjectURL(url); return; }
    if (logoObjectUrl) URL.revokeObjectURL(logoObjectUrl); logoObjectUrl = url; logoImage = image; logoAspect = parsedAspect || ((image.naturalHeight || image.height || 1) / (image.naturalWidth || image.width || 1)); renderFrame(playhead);
  } catch (error) { ui.status.textContent = `Logo load failed: ${error instanceof Error ? error.message : String(error)}`; }
}
async function loadGoogleFont() {
  const family = model.fontFamily.trim() || 'Anton', token = ++fontLoadToken;
  ui.fontStatus.textContent = `Loading ${family}…`; ui.googleFontStylesheet.href = `https://fonts.googleapis.com/css2?family=${encodeURIComponent(family).replace(/%20/g, '+')}:wght@100;200;300;400;500;600;700;800;900&display=swap`;
...
function applyCanvasSize() {
  canvas.width = model.canvasWidth; canvas.height = model.canvasHeight; ui.canvasReadout.textContent = `${model.canvasWidth} × ${model.canvasHeight}`; syncViewport(); renderFrame(playhead);
}
function syncViewport() {
  const zoom = model.viewportZoom;
  ui.viewportZoom.value = String(zoom); ui.fitBtn.classList.toggle('active', zoom === 'fit');
...
function syncTimeUI() {
  const total = durationMs(); ui.totalTimeReadout.value = `${(total / 1000).toFixed(2)} s`; ui.timelineStatus.textContent = formatTime(total);
  const points = [0, .25, .5, .75, 1]; ui.timelineRuler.innerHTML = points.map((p) => `<span>${formatTime(total * p, false)}</span>`).join('');
  const frames = Math.max(2, Math.min(240, Math.round(total / 1000 * model.fps))); ui.frameEstimate.textContent = String(frames);
  ui.outputSizeReadout.textContent = `${Math.round(model.canvasWidth * model.exportScale)} × ${Math.round(model.canvasHeight * model.exportScale)}`;
}
function syncLogoPicker() {
  ui.logoType.value = model.logoType;
  document.querySelectorAll('[data-logo-type]').forEach((button) => { const on = button.dataset.logoType === model.logoType; button.classList.toggle('active', on); button.setAttribute('aria-checked', String(on)); });
}
function syncCurveUI() {
  const curve = selectedCurve === 'enter' ? model.enterBezier : model.exitBezier;
  [ui.x1.value, ui.y1.value, ui.x2.value, ui.y2.value] = curve.map(String);
  const key = curve.join(','); ui.preset.value = PRESETS[key] ? key : 'custom'; ui.curveTarget.value = selectedCurve; updateCurveViz();
}
function updateCurveViz() {
  const [x1, y1, x2, y2] = selectedCurve === 'enter' ? model.enterBezier : model.exitBezier;
  ...
  ui.curvePath.setAttribute('d', `M${start[0]} ${start[1]} C${p1[0]} ${p1[1]} ${p2[0]} ${p2[1]} ${end[0]} ${end[1]}`); ui.guide1.setAttribute('d', `M${start[0]} ${start[1]} L${p1[0]} ${p1[1]}`); ui.guide2.setAttribute('d', `M${end[0]} ${end[1]} L${p2[0]} ${p2[1]}`); ui.handle1.setAttribute('cx', p1[0]); ui.handle1.setAttribute('cy', p1[1]); ui.handle2.setAttribute('cx', p2[0]); ui.handle2.setAttribute('cy', p2[1]);
}
function syncTimeline() {
  const total = durationMs(), phases = phaseFractions();
  for (const layer of ['logo', 'text']) {
    const [start, end] = resolvedRange(layer), bar = ui.timeline.querySelector(`[data-bar="${layer}"]`), ps = bar.querySelectorAll('.phase');
    bar.style.left = `${total ? start / total * 100 : 0}%`; bar.style.width = `${total ? (end - start) / total * 100 : 100}%`;
    ps[0].style.width = `${phases[0] * 100}%`; ps[1].style.width = `${phases[1] * 100}%`; ps[2].style.width = `${phases[2] * 100}%`;
    const enterHandle = bar.querySelector('.phase-handle.enter-end'), exitHandle = bar.querySelector('.phase-handle.exit-start');
    enterHandle.style.left = `${phases[0] * 100}%`; exitHandle.style.left = `${(phases[0] + phases[1]) * 100}%`;
    const editablePhases = model.flowMode === 'in-hold-out' || model.flowMode === 'in-out'; enterHandle.hidden = !editablePhases; exitHandle.hidden = model.flowMode !== 'in-hold-out';
    const startInput = ui[`${layer}Start`], endInput = ui[`${layer}End`]; startInput.max = String(Math.max(0, total - 10)); endInput.max = String(total); startInput.value = String(Math.round(start)); endInput.value = String(Math.round(end));
    ui[`${layer}TimingLabel`].textContent = `${formatTime(start)}–${formatTime(end)}`;
  }
  updateTimelinePlayhead(); syncTimeUI();
}
function updateTimelinePlayhead() {
  const track = ui.timeline.querySelector('.timing-track'), line = ui.timeline.querySelector('.timeline-playhead'); if (!track || !line) return;
  const tr = ui.timeline.getBoundingClientRect(), rr = track.getBoundingClientRect(); line.style.left = `${rr.left - tr.left + rr.width * playhead}px`;
}
function syncFormFromModel() {
  const fields = ['headline','fontFamily','fontSize','fontWeight','letterSpacing','wordSpacing','textAlign','textTransform','textColor','position','gap','alignment','verticalAlignment','offsetX','offsetY','logoOffsetX','logoOffsetY','textOffsetX','textOffsetY','logoSize','logoColor','flowMode','enterDuration','holdDuration','exitDuration','enterDirection','exitDirection','logoTravel','textTravel','logoScale','textScale','wordStagger','canvasWidth','canvasHeight','bgColor','marginX','marginY','fps','exportScale','gifColors','playbackRate'];
  fields.forEach((id) => { if (ui[id]) ui[id].value = String(model[id]); });
  ['autoFit','showMargins','showGrid','gifLoop','previewLoop'].forEach((id) => { if (ui[id]) ui[id].checked = Boolean(model[id]); });
  ui.wordStaggerOut.value = `${Math.round(model.wordStagger)} ms`; syncLogoPicker(); syncCurveUI();
}
function syncAll({ reloadFont = false, reloadLogo = false } = {}) {
  syncFormFromModel(); applyCanvasSize(); syncTimeline(); syncViewport(); renderFrame(playhead); if (reloadFont) loadGoogleFont(); if (reloadLogo) loadBuiltInLogo(model.logoType);
}
...
function tick(now) {
  ...
  if (model.previewLoop && total > 0) playhead = (elapsed % total) / total; else playhead = clamp(elapsed / total, 0, 1); syncPlayhead();
  if (!model.previewLoop && playhead >= 1) { pause(); return; } rafId = requestAnimationFrame(tick);
}
...
function bindModelInput(id, key = id, { number = false, checkbox = false, onInput = null } = {}) {
  const el = ui[id]; if (!el) return;
  const update = () => { model[key] = checkbox ? el.checked : number ? num(el.value, model[key]) : el.value; if (onInput) onInput(); else { syncTimeline(); renderFrame(playhead); } };
  el.addEventListener('input', update); el.addEventListener('change', () => { update(); commitHistory(); });
}
function setupBindings() {
  ['headline','fontFamily','textAlign','textTransform','textColor','position','alignment','verticalAlignment','enterDirection','exitDirection'].forEach((id) => bindModelInput(id));
  ...
  ['enterDuration','holdDuration','exitDuration'].forEach((id) => bindModelInput(id, id, { number: true, onInput: () => { syncTimeline(); syncPlayhead(); } }));
  bindModelInput('flowMode', 'flowMode', { onInput: () => { syncTimeline(); syncPlayhead(); } });
  bindModelInput('exportScale', 'exportScale', { number: true, onInput: syncTimeUI }); bindModelInput('gifColors', 'gifColors', { number: true, onInput: syncTimeUI }); bindModelInput('gifLoop', 'gifLoop', { checkbox: true, onInput: syncTimeUI });
  bindModelInput('playbackRate', 'playbackRate', { number: true, onInput: () => { if (isPlaying) { playbackOffsetMs = playhead * durationMs(); playbackStartedAt = performance.now(); } } }); bindModelInput('previewLoop', 'previewLoop', { checkbox: true });
  ...
  ui.logoFile.addEventListener('change', async () => {
    const file = ui.logoFile.files?.[0]; if (!file) return; let uploadedAspect = null;
    if (file.type === 'image/svg+xml' || file.name.toLowerCase().endsWith('.svg')) { try { uploadedAspect = aspectFromSvg(await file.text()); } catch {} }
    const url = URL.createObjectURL(file), image = new Image(); image.onload = () => { customLogo = image; logoImage = image; logoAspect = uploadedAspect || ((image.naturalHeight || image.height || 1) / (image.naturalWidth || image.width || 1)); model.logoType = 'custom'; if (logoObjectUrl) URL.revokeObjectURL(logoObjectUrl); logoObjectUrl = url; syncLogoPicker(); renderFrame(playhead); commitHistory(); }; image.onerror = () => { URL.revokeObjectURL(url); ui.status.textContent = 'Could not read that logo file.'; }; image.src = url;
  });
}
...
function setupCanvasControls() {
  ...
  ui.fitBtn.addEventListener('click', () => { model.viewportZoom = 'fit'; syncViewport(); });
  ui.viewportZoom.addEventListener('change', () => { model.viewportZoom = ui.viewportZoom.value === 'fit' ? 'fit' : num(ui.viewportZoom.value, 1); syncViewport(); });
  ui.viewportGridBtn.addEventListener('click', () => { model.showGrid = !model.showGrid; ui.showGrid.checked = model.showGrid; syncViewport(); renderFrame(playhead); commitHistory(); });
  ui.viewportSafeBtn.addEventListener('click', () => { model.showMargins = !model.showMargins; ui.showMargins.checked = model.showMargins; syncViewport(); renderFrame(playhead); commitHistory(); });
}
function setupCurveEditor() {
  ui.curveTarget.addEventListener('change', () => { selectedCurve = ui.curveTarget.value; syncCurveUI(); });
  ui.preset.addEventListener('change', () => { if (ui.preset.value === 'custom') return; const curve = PRESETS[ui.preset.value]; if (!curve) return; model[selectedCurve === 'enter' ? 'enterBezier' : 'exitBezier'] = [...curve]; syncCurveUI(); renderFrame(playhead); commitHistory(); });
  [ui.x1, ui.y1, ui.x2, ui.y2].forEach((el) => el.addEventListener('input', () => { model[selectedCurve === 'enter' ? 'enterBezier' : 'exitBezier'] = [clamp(num(ui.x1.value, .05),0,1),num(ui.y1.value,.9),clamp(num(ui.x2.value,.1),0,1),num(ui.y2.value,1)]; ui.preset.value='custom'; updateCurveViz(); renderFrame(playhead); }));
  [ui.x1, ui.y1, ui.x2, ui.y2].forEach((el) => el.addEventListener('change', commitHistory));
  const toSvgPoint = (event) => { const matrix = ui.curveViz.getScreenCTM(); if (!matrix) return null; const point = ui.curveViz.createSVGPoint(); point.x = event.clientX; point.y = event.clientY; return point.matrixTransform(matrix.inverse()); };
  const begin = (which, event) => { activeDrag = { kind: 'curve', which, before: historySnapshot(), pointerId: event.pointerId }; event.target.setPointerCapture?.(event.pointerId); event.preventDefault(); };
  ui.handle1.addEventListener('pointerdown', (e) => begin(1,e)); ui.handle2.addEventListener('pointerdown', (e) => begin(2,e));
  ui.curveViz.addEventListener('pointermove', (event) => { if (activeDrag?.kind !== 'curve') return; const p = toSvgPoint(event); if (!p) return; const x = clamp((p.x-20)/220,0,1), y = clamp(((150-p.y)/130)*1.5-.25,-2,3), curve = [...(selectedCurve==='enter'?model.enterBezier:model.exitBezier)]; curve[activeDrag.which===1?0:2]=Number(x.toFixed(2)); curve[activeDrag.which===1?1:3]=Number(y.toFixed(2)); model[selectedCurve==='enter'?'enterBezier':'exitBezier']=curve; syncCurveUI(); renderFrame(playhead); });
  const end = () => { if (activeDrag?.kind === 'curve') { activeDrag = null; commitHistory(); } }; ui.curveViz.addEventListener('pointerup',end); ui.curveViz.addEventListener('pointercancel',end);
...
function setupTimeline() {
  ...
  for(const layer of ['logo','text']) {
    const start=ui[`${layer}Start`],end=ui[`${layer}End`];
    start.addEventListener('input',()=>setLayerRange(layer,num(start.value,0),resolvedRange(layer)[1])); end.addEventListener('input',()=>setLayerRange(layer,resolvedRange(layer)[0],num(end.value,durationMs()))); start.addEventListener('change',commitHistory);end.addEventListener('change',commitHistory);
  }
  ui.timeline.addEventListener('pointerdown',(event)=>{
    const phaseHandle=event.target.closest('.phase-handle'), handle=event.target.closest('.timing-handle'), bar=event.target.closest('.timing-bar'), track=event.target.closest('.timing-track');
    if(!bar && !track) return;
    const layer=bar?.dataset.bar || track?.dataset.track, timingTrack=ui.timeline.querySelector(`[data-track="${layer}"]`), rect=timingTrack.getBoundingClientRect(), [start,end]=resolvedRange(layer);
    activeDrag={kind:phaseHandle?'phase':handle?(handle.classList.contains('start')?'start':'end'):bar?'bar':'scrub',layer,rect,x0:event.clientX,start0:start,end0:end,before:historySnapshot(),pointerId:event.pointerId,phase:phaseHandle?.classList.contains('enter-end')?'enter':'exit'};
    (bar||track).setPointerCapture?.(event.pointerId); event.preventDefault();
    if(activeDrag.kind==='scrub'){playhead=clamp((event.clientX-rect.left)/Math.max(1,rect.width),0,1);pause();syncPlayhead();}
  });
  ui.timeline.addEventListener('pointermove',(event)=>{
    if(!activeDrag || !['start','end','bar','scrub','phase'].includes(activeDrag.kind)) return;
    const total=durationMs(), dx=(event.clientX-activeDrag.x0)/Math.max(1,activeDrag.rect.width)*total;
    if(activeDrag.kind==='scrub'){playhead=clamp((event.clientX-activeDrag.rect.left)/Math.max(1,activeDrag.rect.width),0,1);syncPlayhead();return;}
    if(activeDrag.kind==='phase'){
      const bar=ui.timeline.querySelector(`[data-bar="${activeDrag.layer}"]`), br=bar.getBoundingClientRect(), frac=clamp((event.clientX-br.left)/Math.max(1,br.width),0,1), phaseTotal=durationMs();
      if(model.flowMode==='in-out'){model.enterDuration=clamp(Math.round(frac*phaseTotal),50,phaseTotal-50);model.exitDuration=phaseTotal-model.enterDuration;model.holdDuration=0;}
      else if(model.flowMode==='in-hold-out'&&activeDrag.phase==='enter'){model.enterDuration=clamp(Math.round(frac*phaseTotal),50,phaseTotal-model.exitDuration);model.holdDuration=Math.max(0,phaseTotal-model.enterDuration-model.exitDuration);}
      else if(model.flowMode==='in-hold-out'){const exitStart=clamp(Math.round(frac*phaseTotal),model.enterDuration,phaseTotal-50);model.holdDuration=exitStart-model.enterDuration;model.exitDuration=phaseTotal-exitStart;}
      syncFormFromModel();syncTimeline();renderFrame(playhead);return;
    }
    let start=activeDrag.start0,end=activeDrag.end0;
    if(activeDrag.kind==='start')start=clamp(activeDrag.start0+dx,0,end-10); else if(activeDrag.kind==='end')end=clamp(activeDrag.end0+dx,start+10,total); else {const len=activeDrag.end0-activeDrag.start0;start=clamp(activeDrag.start0+dx,0,total-len);end=start+len;}
    setLayerRange(activeDrag.layer,start,end);
  });
...
function setupTabsAndLayers() {
  const titles={typography:'Type',lockup:'Lockup',logo:'Logo',motion:'Motion',canvas:'Canvas',export:'Export'};
  document.querySelectorAll('.tab').forEach((tab)=>tab.addEventListener('click',()=>{document.querySelectorAll('.tab').forEach((x)=>x.classList.toggle('active',x===tab));document.querySelectorAll('.tab-panel').forEach((panel)=>panel.classList.toggle('active',panel.dataset.panel===tab.dataset.tab));ui.inspectorTitle.textContent=titles[tab.dataset.tab]||'Inspector';}));
  const layerTabs={lockup:'lockup',logo:'logo',text:'typography',background:'canvas'};
  document.querySelectorAll('[data-select-layer]').forEach((row)=>row.addEventListener('click',()=>{selectedLayer=row.dataset.selectLayer;document.querySelectorAll('[data-select-layer]').forEach((x)=>x.classList.toggle('selected',x===row));document.querySelector(`.tab[data-tab="${layerTabs[selectedLayer]}"]`)?.click();}));
}

async function exportGif() {
  const s=renderState(),totalFrames=Math.max(2,Math.min(240,Math.round(durationMs()/1000*model.fps))),delay=Math.round(1000/model.fps),scale=model.exportScale;
  ui.exportBtn.disabled=true;ui.topExportBtn.disabled=true;ui.exportProgress.value=0;ui.status.textContent='Loading GIF encoder…';
  try{
    const {GIFEncoder,quantize,applyPalette}=await import('https://unpkg.com/gifenc@1.0.3?module');
    const out=document.createElement('canvas');out.width=Math.round(model.canvasWidth*scale);out.height=Math.round(model.canvasHeight*scale);const outCtx=out.getContext('2d',{willReadFrequently:true}),gif=GIFEncoder();
    for(let i=0;i<totalFrames;i++){const t=i/(totalFrames-1);renderFrame(t,outCtx,out.width,out.height,{hideGuides:true});const rgba=outCtx.getImageData(0,0,out.width,out.height).data,palette=quantize(rgba,model.gifColors,{format:'rgb565'}),indexed=applyPalette(rgba,palette,'rgb565');gif.writeFrame(indexed,out.width,out.height,{palette,delay,repeat:model.gifLoop?0:-1});ui.exportProgress.value=(i+1)/totalFrames;ui.status.textContent=`Encoding ${i+1} / ${totalFrames}…`;if(i%3===0)await new Promise((r)=>setTimeout(r,0));}
    gif.finish();const blob=new Blob([gif.bytes()],{type:'image/gif'});downloadBlob(blob,`anim-${model.canvasWidth}x${model.canvasHeight}.gif`);ui.status.textContent=`Done — ${(blob.size/1024/1024).toFixed(1)} MB GIF.`;
  }catch(error){console.error(error);ui.status.textContent=`Export failed: ${error instanceof Error?error.message:String(error)}`;}finally{ui.exportBtn.disabled=false;ui.topExportBtn.disabled=false;}
}
...
function setupCommands(){
  ui.playBtn.addEventListener('click',play);ui.restartBtn.addEventListener('click',restart);ui.prevFrameBtn.addEventListener('click',()=>stepFrame(-1));ui.nextFrameBtn.addEventListener('click',()=>stepFrame(1));
  ui.scrubber.addEventListener('input',()=>{pause();playhead=clamp(num(ui.scrubber.value,0),0,1);playbackOffsetMs=playhead*durationMs();syncPlayhead();});
  ui.exportBtn.addEventListener('click',exportGif);ui.topExportBtn.addEventListener('click',()=>document.querySelector('.tab[data-tab="export"]')?.click());ui.topExportBtn.addEventListener('dblclick',exportGif);ui.pngBtn.addEventListener('click',exportPng);
  ui.resetBtn.addEventListener('click',resetDefaults);ui.undoBtn.addEventListener('click',undo);ui.redoBtn.addEventListener('click',redo);
  window.addEventListener('resize',()=>{updateTimelinePlayhead();if(model.viewportZoom==='fit')syncViewport();});
  window.addEventListener('keydown',(e)=>{
    const editable=['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName);
    if(e.key==='Escape'&&activeDrag){e.preventDefault();cancelActiveDrag();return;}
    if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='z'){e.preventDefault();if(e.shiftKey)redo();else undo();return;}
    if(e.code==='Space'&&!editable){e.preventDefault();play();}
    if(e.key==='ArrowLeft'&&!editable){e.preventDefault();stepFrame(-1);} if(e.key==='ArrowRight'&&!editable){e.preventDefault();stepFrame(1);}
  });
}

async function initialize(){
  setupBindings();setupCanvasControls();setupCurveEditor();setupTimeline();setupTabsAndLayers();setupCommands();syncAll();syncHistoryButtons();await loadBuiltInLogo('wordmark');loadGoogleFont();syncPlayhead();
